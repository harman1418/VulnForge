...
export default function SslScanner() {
  ...
  const handleScan = async (target) => {
    ...
    setResult(null)
    try {
      const res = await API.get(`/api/ssl/?target=${target}`)
      setResult(res.data)
    } catch (err) {
  ...
  return (
    <ToolLayout title="SSL SCANNER" description="Analyze SSL/TLS configuration and certificates using SSLyze" icon="🔒">
